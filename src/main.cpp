...
int main(int argc, char* argv[]) {
	...
	// 227x227x3 (intput layer) <--> 11x11x3 (filter) => 55x55x48 (output volume)
  	Tensor conv_layer_2 = conv_layer_1.fwdConv(kernel_1, 4, 1);

	//  7x7x3 (intput layer) <--> 11x11x3 (filter) => 55x55x48 (output volume)
    ...
    //conv_layer_2 = Tensor(4, 4);
    //conv_layer_2.addLayer(createMatrixFromFile("layers/input_layer_4x4"));

	//Tensor max_pool_layer_1 = conv_layer_2.fwdMaxPool(5, 5, 2, 0);

    // printing first layer of output volume to see if it actually performed proper max pool operation
    //cout << "[Input matrix]" << endl;
    //conv_layer_2.getLayer(0).print();
    //cout << "\n[Max pool operation]" << endl;
    //max_pool_layer_1.getLayer(0).print();
    //cout << "\nOutput volume ->  " << max_pool_layer_1.getHeight() << "x" << max_pool_layer_1.getWidth() << "x" << max_pool_layer_1.getDepth() << endl;
